...
// Nota: CardFormScreen.tsx (RN) solo soporta creación — no hay modo edición
// en el código fuente móvil. El mapa de rutas del port pide una ruta de edición
// (/tarjetas/:cardId/editar), así que aquí se agrega un modo "editar" apoyado
// en updateCardThunk (ya existente en cardsSlice) para cubrir esa ruta: fija
// el tipo a CRÉDITO (solo se llega aquí desde una tarjeta de crédito existente)
// y deshabilita "Últimos 4" porque updateCard no acepta cambiar ese campo.
export default function CardFormPage() {
  ...
  return (
    <PageShell>
      ...
      {!isEdit ? (
        <div style={{ display: 'flex', flexDirection: 'row', background: colors.surface, borderRadius: radius.pill, padding: 4, marginBottom: spacing.xl }}>
          <Pressable
            ...
            style={{
              ...
              background: kind === 'DEBIT' ? colors.accent : 'transparent',
            }}
          >
            <span style={{ color: kind === 'DEBIT' ? colors.black : colors.textSecondary, fontWeight: 700, fontSize: fontSize.sm }}>Débito</span>
          </Pressable>
          <Pressable
            ...
            style={{
              ...
              justifyContent: 'center',
              borderRadius: radius.pill,
              background: kind === 'CREDIT' ? colors.accent : 'transparent',
            }}
          >
            <span style={{ color: kind === 'CREDIT' ? colors.black : colors.textSecondary, fontWeight: 700, fontSize: fontSize.sm }}>Crédito</span>
          </Pressable>
        </div>
      ) : null}
      ...
      {kind === 'DEBIT' && !isEdit ? (
        ...
        <>
          ...
          {preview ? (
            <div style={{ display: 'flex', flexDirection: 'row', alignItems: 'flex-start', background: colors.accentMuted, borderRadius: radius.input, padding: spacing.lg, marginTop: spacing.lg }}>
      ...
      <div style={{ display: 'flex', flexDirection: 'row', gap: spacing.md }}>
        {CARD_COLORS.map((c) => (
          <Pressable
            key={c}
            onClick={() => setColor(c)}
            style={{ width: 36, height: 36, borderRadius: 18, background: c, border: `2px solid ${color === c ? colors.textPrimary : 'transparent'}` }}
          />
        ))}
      </div>
        ...
        <>
          <div style={{ display: 'flex', flexDirection: 'row', alignItems: 'center', marginTop: spacing.xl, gap: spacing.md }}>
            ...
            <Pressable
              ...
              style={{ width: 44, height: 26, borderRadius: 13, background: hasExistingDebt ? colors.accent : colors.divider, position: 'relative', flexShrink: 0 }}
            >
              <span
                style={{
                  position: 'absolute',
                  top: 3,
                  left: hasExistingDebt ? 21 : 3,
                  width: 20,
                  height: 20,
                  borderRadius: 10,
...
const labelStyle: React.CSSProperties = { color: colors.textSecondary, fontSize: fontSize.sm, fontWeight: 600, marginBottom: spacing.sm, marginTop: spacing.lg };
const hintStyle: React.CSSProperties = { color: colors.textMuted, fontSize: fontSize.xs, marginBottom: spacing.sm, marginTop: -spacing.xs };
const inputStyle: React.CSSProperties = { width: '100%', background: colors.surface, borderRadius: radius.input, padding: spacing.lg, color: colors.textPrimary, fontSize: fontSize.md };
const twoColStyle: React.CSSProperties = { display: 'flex', flexDirection: 'row', gap: spacing.lg };
